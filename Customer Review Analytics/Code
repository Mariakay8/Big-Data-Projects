...
# MAGIC %md
# MAGIC Looking at product categories, we can say that orders which include products that are among the Top 20 categories have higher reviews. This is more flargrant in positive reviews as the gap there is larger.

# COMMAND ----------

# MAGIC %md
# MAGIC ##4.7. Reviews Product Display

# COMMAND ----------

# Check the correlation between number of photos and review scores
photo = train_basetable.where((col('review_score')!='0.0') & (col("avg_photos_qty")>0)).groupBy('review_score').agg(mean('avg_photos_qty'))
photo.display()

# COMMAND ----------

# MAGIC %md
# MAGIC As for website attributes, the more photos are available for a product, the higher the customer review.

# COMMAND ----------

# Check the correlation between product description lenght and review scores
...
# MAGIC %md
# MAGIC Finally, the more description available about a product, the higher the customer review.

# COMMAND ----------

# MAGIC %md
# MAGIC ##4.8. Reviews Payment Method
